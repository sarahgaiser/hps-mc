...
space = FilterBunches(inputs=filter_events.output_files(),
                              filter_no_cuts=True,
                              outputs=['%s_spaced.slcio' % signal_name],
                              filter_event_interval=250)

## Print number of merged events
count_space = LCIOCount(inputs=space.output_files())

## Run simulated events in readout to generate triggers
readout = JobManager(steering='readout',
                     inputs=space.output_files(),
...
ana = HPSTR(inputs=cnv.output_files(), outputs=['signal_pure_pulser_style_ana.root'], cfg='ana')

## Add the components
job.add([filter_events, count_filter, space,
         count_space, readout, count_readout, recon, count_recon, cnv, ana])
