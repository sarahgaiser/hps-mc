...
C
      ...
      COMMON /TO_SPECISA/IS_A_J,IS_A_A,IS_A_L,IS_A_EP,IS_A_EM,IS_A_MP,IS_A_MM,IS_A_B,IS_A_NU,IS_HEAVY,
     . IS_A_ONIUM, do_cuts
...
c there is only one positive muon; check if it passes angle cuts & energy cuts
        if( is_a_mp(i) ) then
        ...
        endif ! is positive muon

c there is a recoil electron and a muon from the vertex

        if( is_a_em(i).and. i.eq.nincoming+1 ) then
        ...
        if( is_a_mm(i).and. i.ne.nincoming+1 ) then
          if( (thetax(p(0,i)) .gt. thetaxminmu).and.(thetax(p(0,i)) .lt. thetaxmaxmu)) then
            if( (thetay(p(0,i)) .gt. thetayminmu).and.(thetay(p(0,i)) .lt. thetaymaxmu)) then
              if( (theta(p(0,i)) .gt. thetaminmu).and.(theta(p(0,i)) .lt. thetamaxmu)) then
                if( (p(0,i) .gt. eminmu).and.(p(0,i) .lt. emaxmu)) then
                  passlm=.true.
                  nlmpass=nlmpass+1
                  jpasslm=i
                endif ! pass momentum cuts
