...
export RUNDIR=/fs/ddn/sdf/scratch/s/sgaiser/tritrig/slic/$JOB_ID

mkdir -p $RUNDIR
cd $RUNDIR

/bin/python3 $HPSMC_DIR/lib/python/hpsmc/job.py run -d $RUNDIR -c $JOBDIR/.hpsmc -i ${JOB_ID} tritrig_prep_and_slic $JOBDIR/jobs.json > $RUNDIR/../logs/job.${JOB_ID}.log
