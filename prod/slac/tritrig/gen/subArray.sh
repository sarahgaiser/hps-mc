...
export FIRST_ID=1200
export JOB_ID=$(($SLURM_ARRAY_TASK_ID+$FIRST_ID))
export JOBDIR=$HPSMC/prod/slac/tritrig/gen
export RUNDIR=/fs/ddn/sdf/scratch/s/sgaiser/tritrig/gen/$JOB_ID

mkdir -p $RUNDIR
cd $RUNDIR

/bin/python3 $HPSMC_DIR/lib/python/hpsmc/job.py run -d $RUNDIR -c $JOBDIR/.hpsmc -i ${JOB_ID} tritrig_gen $JOBDIR/jobs.json > $RUNDIR/../logs/job.${JOB_ID}.log
