...
overlay = JobManager(steering='overlay',
                     inputs=['signal_filt.slcio'],
                     overlay_file=pulser_file_name[0],
                     outputs=['%s.slcio' % signal_pulser_name])

## Overlay signal with pulser data
overlay2 = JobManager(steering='overlay',
                      inputs=['signal_pulser.slcio'],
                      overlay_file=pulser_file_name[1],
                      outputs=['signal_pulser_double.slcio'])

## Space overlaid events
#space_overlay = FilterBunches(inputs=overlay.output_files(),
space_overlay = FilterBunches(inputs=['signal_pulser_double.slcio'],
                              filter_no_cuts=True,
...
job.add([filter_events, count_filter, count_pulser, overlay, overlay2,
         space_overlay, count_space_overlay, readout, count_readout, recon])
