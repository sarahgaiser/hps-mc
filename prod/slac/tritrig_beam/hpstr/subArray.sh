...
export JOBDIR=$HPSMC/prod/slac/tritrig_beam/hpstr
export RUNDIR=$SCRATCH/tritrig_beam/hpstr/$JOB_ID

mkdir -p $RUNDIR
cd $RUNDIR

/bin/python3 $HPSMC_DIR/lib/python/hpsmc/job.py run -o $RUNDIR/../logs/job.${JOB_ID}.out -e $RUNDIR/../logs/job.${JOB_ID}.err -l $RUNDIR/../logs/job.${JOB_ID}.log -d $RUNDIR -c $JOBDIR/.hpsmc -i ${JOB_ID} hpstr $JOBDIR/jobs.json
