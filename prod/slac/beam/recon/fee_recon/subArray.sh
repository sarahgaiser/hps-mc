...
export JOBDIR=$HPSMC/prod/slac/beam/recon/fee_recon
export RUNDIR=$SCRATCH/beam/fee/$JOB_ID

mkdir -p $RUNDIR
cd $RUNDIR

/bin/python3 $HPSMC_DIR/lib/python/hpsmc/job.py run -l $RUNDIR/../logs/job.${JOB_ID}.log -d $RUNDIR -c $JOBDIR/.hpsmc -i ${JOB_ID} readout_recon $JOBDIR/jobs.json > $RUNDIR/../logs/job.${JOB_ID}.log
