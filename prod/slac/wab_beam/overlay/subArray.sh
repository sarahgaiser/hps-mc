...
export JOBDIR=$HPSMC/prod/slac/wab_beam/overlay
export RUNDIR=/fs/ddn/sdf/scratch/s/sgaiser/wab_beam/recon/$JOB_ID

mkdir -p $RUNDIR
cd $RUNDIR

/bin/python3 $HPSMC_DIR/lib/python/hpsmc/job.py run -d $RUNDIR -c $JOBDIR/.hpsmc -i ${JOB_ID} ${JOBDIR}/signal_pulser_readout_recon_job.py $JOBDIR/jobs.json > $RUNDIR/../logs/job.${JOB_ID}.log
