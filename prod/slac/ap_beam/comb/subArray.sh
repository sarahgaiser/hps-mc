...
export RUNDIR=$SCRATCH/ap_beam/comb/$JOB_ID

mkdir -p $RUNDIR
cd $RUNDIR

/bin/python3 $HPSMC_DIR/lib/python/hpsmc/job.py run -d $RUNDIR -e $RUNDIR/../logs/job.${JOB_ID}.err -c $JOBDIR/.hpsmc -i ${JOB_ID} $JOBDIR/ap_beam_job.py $JOBDIR/jobs.json > $RUNDIR/../logs/job.${JOB_ID}.log
